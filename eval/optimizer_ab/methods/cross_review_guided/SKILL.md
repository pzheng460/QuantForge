...
This method is the baseline TiMi loop plus a hard review-derived factor
discipline. It is designed to test whether Claude/Codex bidirectional review
can produce reusable optimization factors instead of one-off trial choices.

Before proposing candidates, classify the strategy and map it to the smallest
applicable factor set. Do not apply every factor blindly.

### Required factor checks

1. **EMA slow-region factor** — for EMA/SMA crossover or trend-following
   strategies, test at least one slower neighborhood candidate such as
   `6/18`, `7/21`, `8/21`, or `8/24` when the baseline uses a faster pair.
   The goal is to reduce whipsaw without starving trade count.
2. **ADX 18-23 regime filter factor** — for trend-following strategies with
   whipsaw, test at least one candidate using ADX in the review-supported
   neighborhood: ADX 18-23, with ADX period in the 10-18 range. Avoid treating
   a single ADX point as proven.
3. **ATR stop factor** — when validation or full-window logs show large
   individual losses, or validation warnings include missing stop loss, test
   an ATR stop candidate around 1.5x ATR. Prefer 1.2x, 1.5x, 2.0x as nearby
   perturbations if budget allows.
4. **Direction asymmetry factor** — when a bull/bear regime is obvious and one
   direction produces concentrated losses, test a direction filter or
   asymmetric long/short rule. For bull regimes, this may be long-only or
   short-disabled while the long-term trend is up.
5. **Position sizing factor** — if the strategy implicitly uses 100% equity or
   drawdown is close to the rejection threshold, reduce risk exposure before
   adding more signal complexity.

### Review-derived selection penalties

Apply these penalties before selecting a final candidate:

- If validation trades < 20, mark the candidate as fragile unless it clearly
  dominates on drawdown and profit factor. Never accept validation trades < 10.
- Penalize single-trade concentration: if one trade explains more than roughly
  50% of fit-window profit, require stronger validation evidence or an explicit
  risk-control change.
- Penalize fit/validation divergence: if fit PF improves much more than
  validation PF, treat the candidate as overfit-prone.
- Prefer candidates where neighboring parameter perturbations keep validation
  PF > 1.0 and MaxDD < 15%.

### Minimum candidate mix

For each trial, test at least three distinct candidates with fit and validation
backtests:

- one parameter-neighborhood candidate,
- one review-factor candidate (ADX 18-23, ATR stop, direction asymmetry, or
  position sizing),
- one nearby perturbation of the leading candidate.

If a factor is not applicable, say why in the final summary and replace it with
the next smallest applicable factor. The final output must identify which
factor(s) actually drove the chosen candidate.
