...
def plot_is_vs_oos(rows, out_path):
    ...
    lo = (
        min(
            min(r["is_sharpe"], r["oos_sharpe"])
            for r in rows
            if r.get("is_sharpe") is not None
        )
        - 1
    )
    hi = (
        max(
            max(r["is_sharpe"], r["oos_sharpe"])
            for r in rows
            if r.get("is_sharpe") is not None
        )
        + 1
    ...
    ax.set_title(
        "IS vs OOS Sharpe per trial (baseline TiMi loop, n={})".format(len(rows))
    )
    ax.legend()
    ax.grid(alpha=0.3)
    fig.tight_layout()
...
def plot_oos_histogram(rows, out_path):
    ...
    ax.axvline(
        statistics.median(oos),
        color="green",
        lw=1.5,
        linestyle="-",
        label=f"median = {statistics.median(oos):+.2f}",
    )
