...
`apps/research/` 是数据层：DuckDB 仓库（`data/market.duckdb`）、美股事件研究，
以及每日多资产研报（加密 / 期权链 / 纯价格技术面）。自带统一 CLI
（`python -m apps.research <命令>`）、每日定时任务（`research-daily.timer`）
与可选的研究日报邮件推送。操作手册见 `.agents/skills/quantforge-research/SKILL.md`，
