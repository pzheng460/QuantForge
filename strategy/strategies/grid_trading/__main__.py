...
Launch the grid trading strategy.

Usage:
    # Show help
    uv run python -m strategy.strategies.grid_trading --help

    # Run strategy (default settings)
    uv run python -m strategy.strategies.grid_trading

    # Run in background with tmux
    tmux new-session -d -s grid "cd /home/pzheng46/NexusTrader && uv run python -m strategy.strategies.grid_trading"
"""
...
from strategy.strategies.grid_trading.live import create_strategy
...
Examples:
  ...
  # Run in background with tmux
  tmux new-session -d -s grid "cd /home/pzheng46/NexusTrader && uv run python -m strategy.strategies.grid_trading 2>&1 | tee /tmp/grid_start.log"
...
Trading Logic:
  ...
  - SHORT → CLOSE when price falls ≥2 lines from peak
    ...
    print("Exchange: Bitget (Paper Trading)")
    ...
    try:
        ...
        print("Credentials loaded from .keys/.secrets.toml")
        print()
