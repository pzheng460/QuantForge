...
│                     模拟交易 (Paper Trading)                     │
│                                                                 │
│  python -m strategy.strategies.hurst_kalman.live --mesa 0       │
│      │                                                          │
...
uv run python strategy/strategies/hurst_kalman/backtest.py

# 指定 Mesa #1, 6个月
uv run python strategy/strategies/hurst_kalman/backtest.py --mesa 1 --period 6m
```
...
```bash
uv run python strategy/strategies/hurst_kalman/backtest.py --full --period 1y
```
...
```bash
# 只跑 Grid Search
uv run python strategy/strategies/hurst_kalman/backtest.py --optimize

# 只跑 Walk-Forward
uv run python strategy/strategies/hurst_kalman/backtest.py --walk-forward

# 只跑市场状态分析
uv run python strategy/strategies/hurst_kalman/backtest.py --regime

# 生成HTML报告
uv run python strategy/strategies/hurst_kalman/backtest.py --report

# 查看已保存的回测结果
uv run python strategy/strategies/hurst_kalman/backtest.py --show-results
```
...
```bash
# 短期验证 (快速检查)
uv run python strategy/strategies/hurst_kalman/backtest.py --heatmap --period 6m

# 标准验证 (推荐)
uv run python strategy/strategies/hurst_kalman/backtest.py --heatmap --period 1y

# 长期验证 (最可靠，但需更多数据)
uv run python strategy/strategies/hurst_kalman/backtest.py --heatmap --period 2y

# 高分辨率扫描 (更精细但更慢)
uv run python strategy/strategies/hurst_kalman/backtest.py --heatmap --heatmap-resolution 20

# 带第三维面板 (探索 mean_reversion_threshold 的影响)
uv run python strategy/strategies/hurst_kalman/backtest.py --heatmap --heatmap-third-param mean_reversion_threshold
```
...
```bash
# 查看所有可用的 Mesa 配置
python -c "from strategy.strategies.hurst_kalman.configs import list_all_configs; list_all_configs()"
```
...
```bash
# 验证 Mesa #0
uv run python strategy/strategies/hurst_kalman/backtest.py --mesa 0 --period 1y

# 验证 Mesa #1
uv run python strategy/strategies/hurst_kalman/backtest.py --mesa 1 --period 1y
```
...
# 使用 Mesa #1
uv run python -m strategy.strategies.hurst_kalman.live --mesa 1
```
...
```bash
# 查看验证报告
uv run python strategy/strategies/hurst_kalman/paper_validate.py

# 查看实时日志
tail -f strategy/strategies/hurst_kalman/hurst_kalman.log
```
