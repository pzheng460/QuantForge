"""
Bollinger Band Mean Reversion Trading Strategy for Bitget.
...
Usage:
    uv run python -m strategy.live.bollinger_band.strategy

Log output:
    strategy/live/bollinger_band/strategy_output.log
"""
...
from strategy.live.bollinger_band.configs import (  # noqa: E402
    BBTradeFilterConfig,
    get_config,
)
from strategy.strategies.bollinger_band.core import BBConfig  # noqa: E402
from strategy.live.bollinger_band.indicator import (  # noqa: E402
    BollingerBandIndicator,
    Signal,
)
from strategy.live.common.performance import PerformanceTracker  # noqa: E402
...
class BollingerBandStrategy(Strategy):
    ...
    def __init__(
        ...
        self._filter = filter_config or BBTradeFilterConfig()

        self._symbols = symbols or self._config.symbols

        # Indicators per symbol
        self._indicators: Dict[str, BollingerBandIndicator] = {}
    ...
    def _process_signal(
        ...
        # Mean reversion close signal
        if signal == Signal.CLOSE:
            if position.side is not None and self._can_close_position(symbol):
                self._close_position(symbol, "Mean reversion to SMA")
            return

        # Buy signal (oversold - price below lower band)
        ...
        # Sell signal (overbought - price above upper band)
...
#
# Generate configs: uv run python strategy/live/bollinger_band/backtest.py --heatmap
# List configs:     python -m strategy.live.bollinger_band.configs
# Override:         python -m strategy.live.bollinger_band.strategy --mesa 1
# =============================================================================
