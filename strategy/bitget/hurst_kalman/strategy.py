...
class HurstKalmanStrategy(Strategy):
    ...
    def set_config_info(self, level: int, name: str) -> None:
    ...
    def on_start(self) -> None:
        ...
        self.log.info(f"  mean_reversion_threshold: {self._config.mean_reversion_threshold}")
        ...
        self.log.info(f"  min_holding_bars: {self._filter.min_holding_bars} ({self._filter.min_holding_bars * 15} min)")
        self.log.info(f"  cooldown_bars: {self._filter.cooldown_bars} ({self._filter.cooldown_bars * 15} min)")
    ...
    def _reset_daily_stats(self) -> None:
        ...
        if self._daily_stats.date != today:
            ...
            self._daily_stats = DailyStats(
                date=today,
                starting_balance=balance if balance > 0 else 10000.0,  # Default for demo
            )
    ...
    def _init_performance_tracker(self) -> bool:
        ...
        self.log.info(f"  Config: Level {self._config_level} ({self._config_name})")
        self.log.info("=" * 60)
        return True
    ...
    def on_kline(self, kline: Kline) -> None:
        ...
        # Get position info
        position = self._positions.get(symbol)
        bars_held = current_bar - position.entry_bar if position and position.side else 0

        # Log current state
        cooldown_remaining = max(0, self._cooldown_until.get(symbol, 0) - current_bar)
        pos_str = f"{position.side.value}@{position.entry_price:.0f}" if position and position.side else "FLAT"
        ...
        if self._performance and current_bar - self._last_stats_bar >= self._stats_log_interval:
    ...
    def _close_position(self, symbol: str, reason: str, force: bool = False) -> None:
        ...
        if bookl1:
            exit_price = float(bookl1.bid if position.side == OrderSide.BUY else bookl1.ask)
...
# Level 3: Moderate          - Medium risk (NOT recommended - loses money)
# Level 4: Aggressive        - High risk (NOT recommended - loses money)
# Level 5: Short-Term        - Low-medium risk, optimized for short-term gains
#
# Run `python -m strategy.bitget.hurst_kalman.configs` to see all options
# =============================================================================

SELECTED_CONFIG = 5  # Select level 1-5 (2=Long-term, 5=Short-term recommended)
...
strategy.set_config_info(SELECTED_CONFIG, selected.name)
