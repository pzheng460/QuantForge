...
Usage:
    uv run python -m strategy.bitget.grid

Log output:
    strategy/bitget/grid/grid_output.log
"""
...
from strategy.bitget.grid.indicator import (  # noqa: E402
    GridIndicator,
    GridSignal,
)
...
class PositionState:
    ...
    entry_level: int = 0  # Grid level where position was opened
...
class GridStrategy(Strategy):
    ...
    # Out-of-grid protection: close position if price stays outside grid for this many bars
    _OUT_OF_GRID_MAX_BARS: int = 3  # 3 hours at 1h timeframe

    # Position reconciliation interval (seconds)
    _RECONCILE_INTERVAL_S: float = 60.0  # Check every 60 seconds
    # Max allowed position size (BTC) — emergency brake
    _MAX_POSITION_SIZE: float = 0.5  # Way above normal 0.13, but catches accumulation bugs
    ...
    def __init__(self, symbols: Optional[List[str]] = None):
        ...
        # Out-of-grid bar counter
        self._out_of_grid_bars: Dict[str, int] = {}
        
        # Position reconciliation
        self._last_reconcile_at: float = 0.0
        self._reconcile_mismatch_count: Dict[str, int] = {}  # Consecutive mismatches
    ...
    def _manual_warmup(self, symbol: str, indicator: GridIndicator) -> None:
        ...
        try:
            ...
            # Log initial grid state after warmup
    ...
    def _sync_positions_on_start(self) -> None:
        ...
        for symbol in self._symbols:
            try:
                ...
                # Map exchange position to internal state
                ...
                pos_state.entry_level = 0  # Will be updated when grid calculates
    ...
    def _check_realtime_stops(self, symbol: str, price: float) -> None:
        ...
        if self._should_stop_loss(position.entry_price, price, position.side == OrderSide.BUY):
            self.log.warning(f"STOP LOSS triggered (real-time) for {symbol} @ {price:.1f}")
            self._close_position(symbol, "Stop loss (real-time)", price)

    def _should_stop_loss(self, entry_price: float, current_price: float, is_long: bool) -> bool:
    ...
    def _is_kline_stale(self, kline) -> bool:
        """Check if kline is too old."""
        return self._get_kline_age_s(kline) > self._MAX_KLINE_AGE_S
    ...
    def _reconcile_position(self, symbol: str, price: float) -> None:
        ...
        Fixes:
        - Internal says FLAT but exchange has a position (orphan position)
        - Internal says LONG/SHORT but exchange has nothing (phantom position)
        - Internal size doesn't match exchange size (accumulation bug)
        - Position exceeds _MAX_POSITION_SIZE emergency brake
        """
        ...
        try:
            # Query exchange position via REST API
            ex_side, ex_size, ex_entry = self._fetch_exchange_position(symbol)
        except Exception as e:
            self.log.warning(f"{symbol} | Reconcile: failed to fetch exchange position: {e}")
            return
        ...
        # Determine exchange side as OrderSide
        ex_order_side = None
        ...
        # Compare
        sides_match = (int_side == ex_order_side) or (int_side is None and ex_order_side is None)
        size_match = abs(int_size - ex_size) < 0.001  # Tolerance for rounding

        if sides_match and size_match:
            self._reconcile_mismatch_count[symbol] = 0
            return  # All good

        # Mismatch detected — require 2 consecutive mismatches to act (avoid transient states)
        prev_mismatches = self._reconcile_mismatch_count.get(symbol, 0)
        self._reconcile_mismatch_count[symbol] = prev_mismatches + 1

        self.log.warning(
            f"{symbol} | ⚠️ RECONCILE MISMATCH #{self._reconcile_mismatch_count[symbol]}: "
        ...
        if self._reconcile_mismatch_count[symbol] < 2:
            return  # Wait for second consecutive mismatch to confirm
        ...
        if ex_order_side is None and int_side is not None:
            # Exchange has no position but we think we do → reset internal state
            self.log.error(f"{symbol} | RECONCILE: Exchange FLAT, clearing internal {int_side.value} state")
            position.side = None
            position.entry_price = 0.0
            position.amount = Decimal("0")
            position.entry_time = None
            position.entry_level = 0

        elif ex_order_side is not None and int_side is None:
            # Exchange has position but we think we're flat → sync to exchange
            self.log.error(
            ...
            # Sides don't match — this is very bad, emergency close
            self.log.error(
            ...
            # Same side but different size — sync to exchange size
            self.log.error(
        ...
        # Emergency brake: position too large
    ...
    def _fetch_exchange_position(self, symbol: str) -> tuple:
        ...
        Returns: (side: str|None, size: float, entry_price: float)
        """
        ...
        for pos in positions:
            ...
            if total > 0:
                side = pos.get("holdSide", "")  # "long" or "short"
                entry = float(pos.get("openPriceAvg", "0"))
                return (side, total, entry)
        
        return (None, 0.0, 0.0)

    def _emergency_close_exchange(self, symbol: str, side: str, size: float, price: float) -> None:
    ...
    def _check_out_of_grid(self, symbol: str, indicator, price: float) -> None:
        ...
        if is_outside:
            ...
            if bars_out >= self._OUT_OF_GRID_MAX_BARS:
                ...
                self._close_position(symbol, f"Out-of-grid ({bars_out} bars)", price)
                self._out_of_grid_bars[symbol] = 0
        else:
            self._out_of_grid_bars[symbol] = 0
    ...
    def on_kline(self, kline: Kline) -> None:
        ...
        if len(tick_times) >= 3:
            ...
            if time_span < 2.0:
                self._tick_burst_cooldown[symbol] = now_mono + 30.0  # 30s cooldown (was 10s)
                self.log.warning(
        ...
        # ===== GUARD 3: Stale data check on ALL ticks (not just new bars) =====
        ...
        # Clear stale burst if we just exited stale mode
        if self._stale_burst_count.get(symbol, 0) > 0:
            burst_size = self._stale_burst_count[symbol]
            self.log.info(f"{symbol} | Stale burst ended ({burst_size} ticks), settling 30s...")
            self._stale_burst_count[symbol] = 0
            self._burst_settle_until[symbol] = now_mono + 30.0  # 30s settle (was 5s)
        ...
        # Stop-loss must NEVER be blocked by any filter
        self._check_realtime_stops(symbol, price)

        # ===== GUARD 4: Price spike rejection (but stop-loss already checked) =====
        ...
        # Let indicator handle the kline
        if not indicator:
            return
        
        # Sync preserve_peak_trough flag before indicator processes kline
        position = self._positions.get(symbol)
        indicator._preserve_peak_trough = (position is not None and position.side is not None)
        ...
        if not is_new_bar:
            ...
            # Periodic reconciliation (runs every _RECONCILE_INTERVAL_S)
            if self._check_live_ready(symbol):
            ...
            if now_mono - last_tick_log >= 30:
                ...
                if indicator.grid_lines is not None:
                    grid_str = f"[{indicator.grid_lower:.1f}-{indicator.grid_upper:.1f}]"
                self.log.info(
    ...
    def _process_grid_signals(self, symbol: str, indicator: GridIndicator, price: float) -> None:
        ...
        if position.side is not None:
            if self._should_stop_loss(position.entry_price, price, position.side == OrderSide.BUY):
                ...
                self._close_position(symbol, "Stop loss", price)
                return

        # Grid signal logic from the requirements
        if position.side is None:
            # FLAT - look for entry signals
            
            # BUY signal: price dropped >= entry_lines AND current_level <= grid_count//2
            if (peak_level - current_level >= self._ENTRY_LINES and 
                current_level <= self._GRID_COUNT // 2):
                self._open_position(symbol, OrderSide.BUY, price, current_level)
                return
            
            # SELL signal: price rose >= entry_lines AND current_level >= grid_count//2  
            if (current_level - trough_level >= self._ENTRY_LINES and
                current_level >= self._GRID_COUNT // 2):
                self._open_position(symbol, OrderSide.SELL, price, current_level)
                return
                
        else:
            # In position - look for exit signals
            
            # LONG exit: price rose >= profit_lines from trough
            if (position.side == OrderSide.BUY and
                current_level - trough_level >= self._PROFIT_LINES):
                self.log.info(f"LONG profit target hit: level rose {current_level - trough_level} from trough")
                self._close_position(symbol, "Long profit target", price)
                return
            
            # SHORT exit: price fell >= profit_lines from peak
            if (position.side == OrderSide.SELL and  
                peak_level - current_level >= self._PROFIT_LINES):
                self.log.info(f"SHORT profit target hit: level fell {peak_level - current_level} from peak")
                self._close_position(symbol, "Short profit target", price)
                return

    def _open_position(self, symbol: str, side: OrderSide, price: float, level: int) -> None:
        """Open a new position."""
        ...
        position.entry_time = datetime.now(timezone.utc)
        position.entry_level = level

        # Reset peak/trough to current level so exit conditions
        # measure from position entry, not from stale FLAT-period values
        indicator = self._indicators.get(symbol)
        if indicator:
            indicator._peak_level = indicator.current_level
            indicator._trough_level = indicator.current_level

    def _close_position(self, symbol: str, reason: str, price: float = 0.0) -> None:
        ...
        # Reset peak/trough tracking after closing position
        indicator = self._indicators.get(symbol)
        if indicator:
            indicator._peak_level = indicator.current_level
            indicator._trough_level = indicator.current_level
...
def create_strategy() -> GridStrategy:
    """Create and configure the grid strategy."""
    strategy = GridStrategy(symbols=["BTCUSDT-PERP.BITGET"])
    return strategy
