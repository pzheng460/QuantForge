"""Parity test: RegimeEMASignalCore vs signal generator.

Verifies that the RegimeEMASignalGenerator (which delegates to RegimeEMASignalCore)
produces consistent results when compared to running the core directly.
"""

import numpy as np
import pandas as pd
import pytest

from strategy.indicators.regime_ema import RegimeEMASignalCore
from strategy.strategies.regime_ema.core import RegimeEMAConfig
from strategy.strategies.regime_ema.signal import (
    RegimeEMASignalGenerator,
    RegimeEMATradeFilterConfig,
)


def _generate_trending_ohlcv(n: int = 2000, seed: int = 42) -> pd.DataFrame:
    """Generate synthetic OHLCV data with trending/ranging regimes."""
    rng = np.random.RandomState(seed)

    close = np.zeros(n)
    close[0] = 100.0

    for i in range(1, n):
        regime = (i // 200) % 3
        if regime == 0:
            drift = 0.002  # Stronger uptrend
        elif regime == 1:
            drift = -0.002  # Stronger downtrend
        else:
            drift = 0.0  # Range
        close[i] = close[i - 1] * (1 + drift + rng.normal(0, 0.012))

    spread = rng.uniform(0.005, 0.02, n) * close
    high = close + spread * rng.uniform(0.3, 1.0, n)
    low = close - spread * rng.uniform(0.3, 1.0, n)
    volume = rng.uniform(500, 5000, n) * (1 + rng.uniform(0, 2, n))

    return pd.DataFrame(
        {
            "open": close * (1 + rng.normal(0, 0.002, n)),
            "high": high,
            "low": low,
            "close": close,
            "volume": volume,
        }
    )


class TestRegimeEMASignalCoreParity:
    """Test RegimeEMASignalCore produces consistent signals via generator."""

    def test_generator_uses_core(self):
        """RegimeEMASignalGenerator should produce identical results to direct core usage."""
        data = _generate_trending_ohlcv(1500, seed=42)
        config = RegimeEMAConfig()
        filter_config = RegimeEMATradeFilterConfig()

        gen = RegimeEMASignalGenerator(config, filter_config)
        gen_signals = gen.generate(data)

        core = RegimeEMASignalCore(
            config,
            min_holding_bars=filter_config.min_holding_bars,
            cooldown_bars=filter_config.cooldown_bars,
            signal_confirmation=filter_config.signal_confirmation,
        )
        core_signals = np.zeros(len(data))
        for i in range(len(data)):
            core_signals[i] = core.update(
                close=data["close"].values[i],
                high=data["high"].values[i],
                low=data["low"].values[i],
            )

        np.testing.assert_array_equal(gen_signals, core_signals)

    @pytest.mark.parametrize("seed", [1, 17, 99, 123, 456])
    def test_random_data_parity(self, seed):
        """Parity across multiple random datasets."""
        data = _generate_trending_ohlcv(1500, seed=seed)
        config = RegimeEMAConfig()
        filter_config = RegimeEMATradeFilterConfig()

        gen = RegimeEMASignalGenerator(config, filter_config)
        gen_signals = gen.generate(data)

        core = RegimeEMASignalCore(
            config,
            min_holding_bars=filter_config.min_holding_bars,
            cooldown_bars=filter_config.cooldown_bars,
            signal_confirmation=filter_config.signal_confirmation,
        )
        core_signals = np.zeros(len(data))
        for i in range(len(data)):
            core_signals[i] = core.update(
                close=data["close"].values[i],
                high=data["high"].values[i],
                low=data["low"].values[i],
            )

        mismatches = np.where(gen_signals != core_signals)[0]
        assert len(mismatches) == 0, (
            f"Seed {seed}: {len(mismatches)} mismatches. "
            f"First 5: {mismatches[:5].tolist()}"
        )

    def test_custom_config_parity(self):
        """Parity with non-default config parameters."""
        data = _generate_trending_ohlcv(1500, seed=77)
        config = RegimeEMAConfig(
            fast_period=10,
            slow_period=30,
            atr_period=10,
            adx_period=10,
            adx_trend_threshold=20.0,
            stop_loss_pct=0.05,
        )
        filter_config = RegimeEMATradeFilterConfig(
            min_holding_bars=3, cooldown_bars=1, signal_confirmation=2
        )

        gen = RegimeEMASignalGenerator(config, filter_config)
        gen_signals = gen.generate(data)

        core = RegimeEMASignalCore(
            config, min_holding_bars=3, cooldown_bars=1, signal_confirmation=2
        )
        core_signals = np.zeros(len(data))
        for i in range(len(data)):
            core_signals[i] = core.update(
                close=data["close"].values[i],
                high=data["high"].values[i],
                low=data["low"].values[i],
            )

        np.testing.assert_array_equal(gen_signals, core_signals)

    def test_signals_have_trades(self):
        """Sanity check: the generated data should produce some trades."""
        # Regime EMA needs strong trends with sufficient regime detection warmup.
        # Use stronger drift and lower noise for clearer regime signals.
        rng = np.random.RandomState(42)
        n = 3000
        close = np.zeros(n)
        close[0] = 100.0
        for i in range(1, n):
            regime = (i // 300) % 3
            if regime == 0:
                drift = 0.004  # Strong uptrend
            elif regime == 1:
                drift = -0.004  # Strong downtrend
            else:
                drift = 0.0
            close[i] = close[i - 1] * (1 + drift + rng.normal(0, 0.008))

        spread = rng.uniform(0.005, 0.02, n) * close
        high = close + spread * rng.uniform(0.3, 1.0, n)
        low = close - spread * rng.uniform(0.3, 1.0, n)
        volume = rng.uniform(500, 5000, n) * (1 + rng.uniform(0, 2, n))
        data = pd.DataFrame(
            {
                "open": close * (1 + rng.normal(0, 0.002, n)),
                "high": high,
                "low": low,
                "close": close,
                "volume": volume,
            }
        )

        config = RegimeEMAConfig(
            fast_period=10,
            slow_period=25,
            adx_period=10,
            adx_trend_threshold=15.0,
            trend_atr_threshold=0.8,
        )
        filter_config = RegimeEMATradeFilterConfig()

        gen = RegimeEMASignalGenerator(config, filter_config)
        signals = gen.generate(data)

        non_hold = np.count_nonzero(signals)
        assert non_hold > 0, "No trades generated — test data may need adjustment"
