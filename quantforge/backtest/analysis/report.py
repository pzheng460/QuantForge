...
<body>
    ...
    def _generate_equity_section(self) -> str:
        ...
        if self.bh_equity_curve is not None:
            bh_values = self.bh_equity_curve.reindex(
        ...
        subtitle_html = (
            f'<p style="color:#666;font-size:0.9em;margin-bottom:8px">{subtitle}</p>'
            if subtitle
            else ""
        )
    ...
    def _generate_footer(self) -> str:
        ...
        return f"""
        <footer>
            <p>Generated by QuantForge Backtest Engine | {self.result.run_time.strftime("%Y-%m-%d %H:%M:%S")}</p>
