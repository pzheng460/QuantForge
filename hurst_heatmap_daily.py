"""Hurst-Kalman heatmap on DAILY data to avoid OOM."""
import asyncio, itertools, io, contextlib
...
PARAMS = list(itertools.product(
    [1.5, 2.0, 2.5, 3.0, 3.5, 4.0],  # zscore_entry
    [20, 30, 50, 70, 100],             # hurst_window
    [0.45, 0.50, 0.55],               # mr_threshold
    [0.1, 0.2, 0.5],                  # kalman_R
    [0.03, 0.05, 0.07, 0.10],         # stop_loss
))

async def main():
    # Use daily data: ~1500 bars instead of 36k
    ...
    for i, (zs, hw, mrt, kr, sl) in enumerate(PARAMS):
        cfg = HurstKalmanConfig(
            ...
            kalman_R=kr, stop_loss_pct=sl, timeframe="1d",
        )
        ...
        with contextlib.redirect_stdout(f):
            try:
                m = runner.run_single(data=data, config_override=cfg, filter_override=filt)
            except:
                continue
    ...
    print(f"\n{'='*80}")
    print(f"TOP 20 (by Sharpe) — {len(results)} combos tested")
    print(f"{'='*80}")
