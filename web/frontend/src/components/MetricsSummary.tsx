...
function parseMetricsFromEvents(events: AgentEvent[], metrics: AgentMetric[]): ParsedMetric[] {
  ...
  for (const event of events) {
    // Scan ALL event types for metrics
    ...
    // Parse metrics from any event content
      ...
      <div className="flex items-center justify-between">
        <span
          className={cn(
            ...
            improved ? 'text-green-400' : regressed ? 'text-red-400' : 'text-foreground',
          )}
        >
        ...
        {previous && (
          <div className="text-xs">
            <span
              className={cn(
                'font-medium',
                improved ? 'text-green-400' : regressed ? 'text-red-400' : 'text-muted-foreground',
              )}
            >
              {improved ? '\u2197' : regressed ? '\u2198' : '\u2192'}{' '}
  ...
  return (
    <div className={cn('flex flex-col', className)}>
      <div className="p-4 border-b border-border">
        ...
        <p className="text-xs text-muted-foreground mt-1">Auto-extracted from agent output</p>
      </div>
